...
class KNN:
    ...
    def predict(self, features):
        ...
        for i in range(len(features)):
            n_labels = self.get_k_neighbors(features[i])

            if sum(n_labels) > self.k/2:
    ...
    def get_k_neighbors(self, point):
        ...
        for i in range(len(self.features)):
            D[i] = self.distance_function(point, self.features[i])
        D = sorted(D.items(), key = lambda item:item[1])
        k_neighbor = D[:self.k]
        k_neighbor_label = []
        for i in range(self.k):
            k_neighbor_label.append(self.labels[k_neighbor[i][0]])
